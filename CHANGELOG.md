...
### 改进
- 分离播放和搜索命令处理
- `点歌x`（不带空格）使用 `filter.regex` 处理，播放第x首歌曲
- `点歌 xxx`（带空格）使用 `filter.command` 处理，搜索歌曲
- 如果用户输入 `点歌 1`（带空格的数字），会提示使用正确格式

## 1.1.3 (2025-02-16)

### 修复
- 修复filter.command参数处理问题
- AstrBot的filter.command会自动去掉命令前缀，只传递参数
- 现在正确处理纯数字参数（播放模式）和关键词参数（搜索模式）
...
### 修复
- 修复点歌命令逻辑：`点歌17`播放第17首，`点歌 17`搜索关键词"17"
- 播放歌曲必须使用"点歌x"格式（不带空格），搜索可以带空格

## 1.1.0 (2025-02-16)

### 修复
- 修复asyncio.Lock在__init__中创建导致的问题，改为惰性初始化
- 修复_update_session_timestamp死锁问题
- 修复terminate()函数中使用锁导致无法终止的问题
- 合并search_song和play_song_by_number为handle_dian_ge，解决命令冲突

### 改进
- 点歌命令现在支持"点歌 17"格式（带空格）
- 所有异步锁和信号量在事件循环中创建，避免潜在问题
